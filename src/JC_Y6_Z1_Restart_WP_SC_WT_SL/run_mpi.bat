...
echo "**Run Starter**"
%OPENRADIOSS_PATH%\exec\starter_win64.exe -i %input%_0000.rad -np %np% >starter.out 
echo "**Run Engine**"
%MPI_PATH%\mpiexec.exe -delegate -np %np% %OPENRADIOSS_PATH%\exec\engine_win64_impi.exe -i %input%_0001.rad  >engine.out
echo "Done"
